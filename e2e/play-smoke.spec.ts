...
test('post-Chapter IV chapters screen invites the Machine of Discipline', async ({ page }) => {
  ...
  await expect(page.locator('.roadmap-teaser').first()).toContainText(/Sovereign exchange|Alexandrine/i)
...
test('post-Chapter VII chapters screen shows mastery plateau hub', async ({ page }) => {
  await page.addInitScript(() => {
    const save = {
      version: 3,
      chapterIndex: 7,
  ...
  await expect(page.locator('.plateau-hub')).toContainText('I–VII are sealed')
  await expect(page.locator('#btn-plateau-duel')).toBeVisible()
...
test('Chapter VI drills solve on the live board', async ({ page }) => {
  ...
  await expect(page.locator('#btn-next')).toBeEnabled()
  await page.locator('#btn-next').click()
  await expect(page.locator('#narrative-body')).toContainText(/queen|hanging|precision/i)
...
test('Chapter VII drills solve on the live board', async ({ page }) => {
  ...
  await expect(page.locator('#btn-next')).toBeEnabled()
  await page.locator('#btn-next').click()
  await expect(page.locator('#narrative-body')).toContainText(/castle|safer wing|queenside/i)
  await page.locator('[data-square="e1"]').click()
