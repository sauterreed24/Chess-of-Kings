...
describe('mainUiFormatters', () => {
  ...
  it('spokenLineText exposes full AT text while hiding animated character spans', () => {
    ...
    expect(html).toContain('class="spoken-word"')
    expect(html).not.toContain('style="display:inline-block"')
  ...
  it('capturedRow paints board-skin SVGs instead of unicode glyphs', () => {
    ...
    expect(capturedRow(['q'], 'w', 'alexandrine-ornate')).toContain('width:2rem')
