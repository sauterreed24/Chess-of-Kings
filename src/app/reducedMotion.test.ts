...
describe('Alexandrine Imperial state polish', () => {
  ...
  it('keeps the last move legible as an origin-to-destination route cue', () => {
    expect(CSS).toMatch(/\.sq-last-from\s*\{[^}]*box-shadow:\s*inset 0 0 0 5px #a8d88acc/s)
    expect(CSS).toMatch(/\.sq-last-from::before\s*\{[^}]*#1a3a5c4d/s)
    expect(CSS).toMatch(/\.sq-last-to(?:,\s*\.sq-legal-castle)?\s*\{[^}]*inset 0 0 0 5px #f0d28af5/s)
    expect(CSS).toMatch(/\.sq-legal-capture\s*\{[^}]*inset 0 0 0 5px #e8c97ef5/s)
