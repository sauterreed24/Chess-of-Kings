...
describe('GameFlow depth systems', () => {
  ...
  it('keeps Edred\'s fire aim when a pawn is selected', () => {
    let latest: { boardGuide: string } | null = null
    const flow = new GameFlow(PLAYABLE_CHAPTERS, {
      onSceneChange: vi.fn(),
      onChessUpdate: (payload) => {
    ...
    f.highestUnlockedChapter = 1
    const ch1 = PLAYABLE_CHAPTERS.findIndex((c) => c.id === 'ch1')
    const matchIdx = PLAYABLE_CHAPTERS[ch1]!.scenes.findIndex((s) => s.id === 'c1-match-edred')
    expect(matchIdx).toBeGreaterThanOrEqual(0)
    flow.jumpToScene(ch1, matchIdx)
    expect(latest?.boardGuide).toMatch(/Castle through the fire|deny Edred|open lines/i)
    expect(latest?.boardGuide.length).toBeLessThan(80)
    flow.board?.showLegalFrom(flow.chess, 'e2')
    expect(latest?.boardGuide).toMatch(/Castle through the fire|deny Edred|open lines/i)
    expect(latest?.boardGuide).not.toMatch(/e2 pawn selected/)
  ...
  it('keeps Marius\'s imbalance aim when a pawn is selected', () => {
    let latest: { boardGuide: string } | null = null
    const flow = new GameFlow(PLAYABLE_CHAPTERS, {
      onSceneChange: vi.fn(),
      onChessUpdate: (payload) => {
    ...
    f.highestUnlockedChapter = 1
    const ch1 = PLAYABLE_CHAPTERS.findIndex((c) => c.id === 'ch1')
    const matchIdx = PLAYABLE_CHAPTERS[ch1]!.scenes.findIndex((s) => s.id === 'c1-match-marius')
    expect(matchIdx).toBeGreaterThanOrEqual(0)
    flow.jumpToScene(ch1, matchIdx)
    expect(latest?.boardGuide).toMatch(/small imbalance|do not wait for a gift/i)
    expect(latest?.boardGuide.length).toBeLessThan(80)
    flow.board?.showLegalFrom(flow.chess, 'e2')
    expect(latest?.boardGuide).toMatch(/small imbalance|do not wait for a gift/i)
    expect(latest?.boardGuide).not.toMatch(/e2 pawn selected/)
  ...
  it('keeps the Counterpart aim when a knight is selected', () => {
    let latest: { boardGuide: string } | null = null
    const flow = new GameFlow(PLAYABLE_CHAPTERS, {
      onSceneChange: vi.fn(),
      onChessUpdate: (payload) => {
    ...
    f.highestUnlockedChapter = 1
    const ch1 = PLAYABLE_CHAPTERS.findIndex((c) => c.id === 'ch1')
    const matchIdx = PLAYABLE_CHAPTERS[ch1]!.scenes.findIndex((s) => s.id === 'c1-boss')
    expect(matchIdx).toBeGreaterThanOrEqual(0)
    flow.jumpToScene(ch1, matchIdx)
    expect(latest?.boardGuide).toMatch(/Stay accountable|no loose pieces/i)
    expect(latest?.boardGuide.length).toBeLessThan(80)
    flow.board?.showLegalFrom(flow.chess, 'g1')
    expect(latest?.boardGuide).toMatch(/Stay accountable|no loose pieces/i)
    expect(latest?.boardGuide).not.toMatch(/g1 knight selected/)
