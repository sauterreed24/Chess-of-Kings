...
import { GameFlow } from './app/gameFlow'
import type { ChessUiPayload, MoveQuality } from './app/gameFlow'
import { clearSave, hasSave } from './app/storage'
import type { Chapter, MatchScene, Scene, PieceSkinId, RewardBundle, MatchHistoryEntry } from './types'
import { PIECE_SKIN_LABEL } from './chess/skins'
import { getBookTopLines } from './chess/openings'
import { escapeHtml } from './app/htmlEscape'
import { buildReplayFens, formatEchoTimeline, renderEchoBoardFen } from './app/chronicleReplay'
import { createRewardOverlayController } from './app/rewardOverlayController'
import { routeEscapeKey } from './app/escapeKeyRouting'
import { createEchoReplayTimer } from './app/chronicleEchoTimer'
...
function sceneTypeLabel(sc: Scene): string {
  switch (sc.type) {
    case 'dialogue':    return 'Dialogue'
    case 'interlude':   return 'Interlude'
    case 'codex':       return sc.heading
    case 'puzzle':      return 'Puzzle'
    case 'match':       return 'Match'
    case 'calibration': return 'Calibration'
    case 'freeplay':    return 'Free board'
  }
}

function teachingBlock(teaching: NonNullable<import('./types').PuzzleScene['teaching']>) {
  return `
    <div class="teaching">
...
function formatMoveLedger(sanLog: string[], sanQuality: MoveQuality[]): string {
  if (sanLog.length === 0) {
...
function gradeScore(g: MatchHistoryEntry['styleGrade']): number {
  if (g === 'S') return 5
  if (g === 'A') return 4
  if (g === 'B') return 3
  if (g === 'C') return 2
...
function dynamicTrainingTitle(seed: number): string {
  const variants = [
    'Next Training Focus',
    'Adaptive Next Steps',
    'Refinement Focus',
...
function performanceDeltaLines(history: MatchHistoryEntry[], latest: MatchHistoryEntry): string[] {
  const prev = history
  ...
  if (wins >= 3) lines.push('Rival pressure trend: sustained dominance detected — increase duel difficulty for sharper growth.')
  else if (wins === 0) lines.push('Rival pressure trend: opponent still dictating terms — use counter-prep before rematch.')

  return lines.slice(0, 3)
}

/* Piece unicode glyphs: index 0 = white (filled), index 1 = black */
...
function capturedRow(types: string[], color: 'w' | 'b'): string {
...
function getCaptured(chess: import('chess.js').Chess): { byWhite: string[]; byBlack: string[] } {
  ...
  for (const t of ['q', 'r', 'b', 'n', 'p']) {
    ...
    for (let i = 0; i < bLost; i++) byWhite.push(t) // white took black pieces
    for (let i = 0; i < wLost; i++) byBlack.push(t) // black took white pieces
  }
  return { byWhite, byBlack }
}

function diffStars(difficulty: number | undefined): string {
...
function tierLabel(tier: MatchScene['ladderTier']): string {
  const map: Record<string, string> = {
    initiate:    'Initiate',
