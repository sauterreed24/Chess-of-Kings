...
describe('inferRivalIdFromSceneId', () => {
  it('matches a campaign scene id containing a rival key', () => {
    ...
    expect(inferRivalIdFromSceneId('c6-match-prax')).toBe('prax')
    ...
    expect(inferRivalIdFromSceneId('c9-match-wren')).toBe('wren')
    expect(inferRivalIdFromSceneId('c9-match-bram')).toBe('bram')
