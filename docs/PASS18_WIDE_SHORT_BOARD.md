...
1. **Stack only when narrow** — the board-first stack stays at `max-width: 960px`. Short height alone no longer collapses a wide atelier.
2. **Wide short restore** — below 620px tall and at least 961px wide, the live lab keeps manuscript | instrument columns and sizes the marble with `--mobile-board-max` (72vh fallback, not 42vh).
